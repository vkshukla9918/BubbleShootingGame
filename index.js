...
const killBubbleSound = new Audio("./music/killBubble.mp3");
...
// -------------------------------creating Player, Weapon, Bubble classes-----------------------------------
...
// *Creating Player Object, Weapons Array, Bubble Array etc Array

const shubham = new Player(playerPosition.x, playerPosition.y, 15, "white");
const weapons = []; 
const enemies = []; 
 
// *Function To Spawn Bubble (producing offspring) at Random Location 

const spawnBubble = () => 
{
  const BubbleSize = Math.random() * (40-5) + 5;                             // generating random size for Bubble
  const BubbleColor = `hsl(${Math.floor(Math.random() * 360)}, 100%, 50%)`;  // generating random color for Bubble

  let random;                                                               // random is Bubble spawn position
  if (Math.random() < 0.5)                                                  // making Bubble's initial location  
  {                                                                         // making X very left/right off of screen & Y any 
    random = { x: Math.random()<0.5 ? canvas.width+BubbleSize : 0-BubbleSize, y: Math.random()*canvas.height, };
  } 
  else 
  {                                                                         // making X any & Y very up/down off of screen 
    random = { x: Math.random()*canvas.width, y: Math.random()<0.5 ? canvas.height+BubbleSize : 0-BubbleSize, };
  }
 
  const myAngle = Math.atan2( canvas.height/2 - random.y, canvas.width/2 - random.x );    // angle between center & Bubble 
  const velocity = { x: Math.cos(myAngle)*difficulty, y: Math.sin(myAngle)*difficulty, }; // velocity based on difficulty 
  enemies.push(new Bubble(random.x, random.y, BubbleSize, BubbleColor, velocity));           // adding Bubble to enemies array
};
...
function animation() 
{
  ...
  enemies.forEach((Bubble, BubbleIndex) =>                                    // generating enemies
  { 
    Bubble.update();                                          
                                                                            // finding distance between player and Bubble
    const distanceBetweenPlayerAndBubble = Math.hypot( shubham.x-Bubble.x, shubham.y-Bubble.y );  
 
    if (distanceBetweenPlayerAndBubble-shubham.radius-Bubble.radius < 1)     // stopping Game if Bubble hits player
    { cancelAnimationFrame(animationId);

      gameOverSound.play(); 
      shootingSound.pause(); 
      killBubbleSound.pause();
      return gameoverLoader(); }
  
    weapons.forEach((weapon, weaponIndex) => 
    {                                                                       // finding Distance between weapon and Bubble
      const distanceBetweenWeaponAndBubble = Math.hypot( weapon.x-Bubble.x, weapon.y-Bubble.y );  

      if (distanceBetweenWeaponAndBubble-weapon.radius-Bubble.radius < 1) 
      { killBubbleSound.play();

        playerScore += 10;                                                  // finding distance between weapon and Bubble
        scoreBoard.innerHTML = `Score : ${playerScore}`;                    // rendering player Score in scoreboard html element
        setTimeout(() => { enemies.splice(BubbleIndex, 1); weapons.splice(weaponIndex, 1); }, 0); }  
    });
  });
}
